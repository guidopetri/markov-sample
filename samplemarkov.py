...
class word():
	def __init__(self,initialDict=None,instanceCount=None):
		if initialDict == None and instanceCount == None:
			self.followDict = {}
			self.allInstances = 0
		else:
	...
	def nextWord(self):
		#sumProb = sum([probability for (count,probability) in self.followDict.values()])
		#print(sumProb)
		if len(self.followDict) == 0: #if the dictionary is empty
			return "."
		else:
			return numpy.random.choice(list(self.followDict.keys()),p=[probability for (count,probability) in self.followDict.values()])
...
try:
	...
	for key in jsonData['words'].keys():
		wordDict[key] = word(initialDict=jsonData['words'][key]['nextWords'],instanceCount=jsonData['words'][key]['instanceCount'])
except IOError:
	...
	for key in wordDict.keys():
		for match in re.finditer(r'\b'+key+r'\b ?(\.|[a-zA-Z]+\b)',text,re.IGNORECASE):
